...
import com.program.haohu.service.admin.*;
import com.program.haohu.util.CpachaUtil;
import com.program.haohu.util.MenuUtil;
...
public class SystemController {
    ...
    public ModelAndView index(ModelAndView model, HttpServletRequest request) {
        ...
        // topMenuList对应system/index.jsp中<c:forEach items="${topMenuList}" var="menu">中的topMenuList
        ...
        // 在页面上赋值：model.addObject("name", "haohu"); 使用时在页面上引用${name }
    ...
    @ResponseBody
    // String cpacha,cpacha为login.jsp页面中ajax异步请求传递过来的。User user，框架自动封装。
    // $.ajax({
    //            url:'login',
    //            // 通过ajax，根据上面jquery定义的验证码变量来获取验证码cpacha，传递给SystemController.java
    //            data:{username:username, newPassword:newPassword, cpacha:cpacha},
    public Map<String, String> loginAct(User user, String cpacha, HttpServletRequest request) {
        ...
        Object loginCpacha = request.getSession().getAttribute("loginCpacha"); // 得到画布上的验证码字符串
        if (loginCpacha == null) {
            ret.put("type", "error");
            ret.put("msg", "会话超时，请刷新页面！");
            return ret;
        }
        // cpacha为输入的验证码，loginCpacha为画布上得到的验证码
    ...
    // edit_password对应于system/edit_password.jsp中$.ajax({
    //        url:'edit_password',中的edit_password
    @RequestMapping(value = "/edit_password", method = RequestMethod.POST)
    @ResponseBody
    // oldPassword对应于system/edit_password.jsp页面中$.ajax({
    //        data:{oldPassword:$("#oldPassword").val(),newPassword:$("#newPassword").val()},中oldPassword:$("#oldPassword").val()中的前一个oldPassword
    ...
    public void generateCpacha(
            // vl,w,h,type为URL中?后面的参数，比如：get_cpacha?vl=4&w=150&h=40&type=loginCpacha
