            ...
            <tr>
                ...
                <td>
					<select name="parentId" class="easyui-combobox" panelHeight="auto" style="width: 268px">
						...
						<!-- topList对应MenuController中的 model.addObject("topList", menuService.findTopList()); 中的topList -->
...
<!-- 选择图标弹窗 -->
<div id="select-icon-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'" style="width: 830px;height: 550px; padding:10px;">
...
<script type="text/javascript">
	...
	*/
	...
	function openAddMenu(){
		...
		<!-- 如果选中菜单的上一级菜单parentId不为0，则选中菜单不是二级菜单 -->
		...
		$('#add-menu-dialog').dialog({
			...
			onBeforeOpen:function() {
				<!-- 与$("#parent-menu").val(item.name); 双引号是一样的 -->
	...
	function openEdit(){
		...
		$('#edit-dialog').dialog({
			...
			onBeforeOpen:function() {
				...
				<!-- 获取选中菜单的上一级菜单 -->
				var parent = $('#data-datagrid').treegrid('getParent', item.id);
				<!-- 如果选中菜单的上一级菜单不为空 且 选中菜单的上一级菜单的parentId不为0，则选中菜单不是二级菜单，而是按钮 -->
				if (parent != null) {
					if (parent.parentId != 0) {
					    // 将按钮的上级菜单设置为按钮上一级菜单parent的名称，且设置为只可读，不可选
	...
	*/
	$('#data-datagrid').treegrid({
		<!-- ../../admin/menu/list对应于MenuController中@RequestMapping(value = "/list", method = RequestMethod.POST)中的list -->
		url:'../../admin/menu/list',
		<!-- 去掉之后刷新功能才有效 -->
